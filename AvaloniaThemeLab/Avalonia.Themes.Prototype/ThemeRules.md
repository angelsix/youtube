...
This Prototype theme replaces that with a **token-based architecture**. A single `DefaultTheme.cs` class defines every design decision — colors, spacing, type scale, corner radii, control heights, thicknesses — as named tokens. These tokens are exposed as markup extensions in three namespaces, chosen by the C# type of the property they wrap: paint types (`Color`, anything implementing `IBrush`) become `{color:...}` (`{color:AccentBrush Light10}`), numeric and geometric types (`double`, `Thickness`, `CornerRadius`, ...) become `{size:...}` (`{size:SpacingMd}`, `{size:RadiusSm}`), and everything else stays `{theme:...}` (`{theme:FontFamily}`, `{theme:ControlHorizontalAlignment}`). Any control theme can reference all three. Every themed file declares the three prefixes on its root element, the two newer ones sitting beside the original `theme` declaration:

```xml
<ResourceDictionary xmlns="https://github.com/avaloniaui"
                    xmlns:theme="using:AngelSix.ThemeEngine.Generated"
                    xmlns:color="using:AngelSix.ThemeEngine.Colors"
                    xmlns:size="using:AngelSix.ThemeEngine.Sizes">
```

> **Spelling is American, everywhere.** The paint namespace is `AngelSix.ThemeEngine.Colors`, the prefix is `{color:...}`, and the ramp attribute is `[ColorRamp]` — never `Colours` / `colour` / `[ColourRamp]`. The engine went American in 1.18.0, so the British forms no longer resolve at all; and beyond compiling, the rest of the ecosystem (Avalonia's own `Color` type, the generated member names, every downstream theme) spells it `color`, so mixing in a `colour` reads as a mistake even where it happens to work. When you write a new themed file, copy the three-line xmlns block above verbatim.

The effect is transformative:

- **Clean, concise XAML syntax.** Instead of referencing resources through string-based keys (`{StaticResource MyColor}`) with no compile-time checking, you write `{color:AccentBrush Light10}`, `{size:SpacingMd}`, `{size:RadiusSm}`. This is valid XAML that the AngelSix.ThemeEngine source generator understands directly.
...
- **No more hunting through XAML for hard-coded colors.** Every visual value flows from a named token with a clear purpose.
...
**Statement:** Every color, font family, font size, and font weight on a UI surface flows from a theme token. Hard-coded values are **not** the default — they are restricted to a small, closed list of exemptions. When you encounter a hard-coded color or font value, replace it with the most suitable theme token. Do not preserve the exact pixel value; pick the token that matches the control's visual role.
...
|-----------|---------|-----|
| **`LinearGradientBrush` / `RadialGradientBrush` with hardcoded stops** | `<GradientStop Offset="0" Color="#4F000000"/>` | Every `GradientStop.Color` must use a theme token: `Color="{color:AccentNeutral}"` (with opacity on the brush or stop) |
| **`SolidColorBrush` with hardcoded hex** | `<SolidColorBrush Color="#FF797774"/>` | Use `Color="{theme:...}"` — e.g. `Color="{color:AccentNeutral}"` |
| **`BoxShadow` with any hardcoded color** | `BoxShadow="0 6 8 0 #4F000000"` | Replace with a `Border` element behind the target using `Background="{color:OverlayWeakBrush}"` (or another overlay brush). See [Shadow Theming](#shadow-theming) below. |
| **Any `#RRGGBB` or `#AARRGGBB` hex on a UI element** | `Fill="#FF000000"`, `Stroke="#80FFFFFF"` | Replace with the appropriate theme brush token |

> **Avalonia caveat:** `GradientStop` has no `Opacity` property — attempting `<GradientStop Opacity="0.05"/>` produces `AVLN2000`. Use a themed brush with built-in opacity (e.g., `{color:OverlayWeakBrush}`) instead, or encode the alpha in the color itself via a token.
...
| **`StrokeDashArray` values** | `StrokeDashArray="3, 2, 6, 2"` | Structural pattern definition, not a color |
...
**Stage properties have no markup extension.** The generator suppresses them, because `{color:AccentBrush Dark2}` already reaches every one and ~460 entries make IntelliSense unusable. Only the base `Accent{Hue}` and `Accent{Hue}Brush` are emitted. There is no color-valued equivalent of `AccentBrush`, so a `GradientStop` or `Pen` can only reach a hue's base.
...
Each paint token has two spellings — `{color:X}` and `{color:XBrush}` — and they are **not always interchangeable**. Which one you may use depends on what the bare form actually reads:

| Token family | Bare form reads | In a paint slot (`Background`, `Foreground`, …) | In a `Color` slot (`GradientStop.Color`, `SolidColorBrush.Color`) |
|---|---|---|---|
| **Roles** (`TextDefault`, `DimTextDefault`, …) | the `…Brush` member itself — identical to the suffixed form | ✅ **Write the bare form.** The suffix is pure noise; `{color:TextDefaultBrush}` is banned. | ❌ Both spellings hand back a brush, not a color. Reach a plain token instead (`{color:AccentNeutral}`). |
| **Plain tokens** (`AccentPrimary`, `SurfaceDefault`, `NeutralDark1`, …) | the `Color` member — a *different type* | ❌ **Keep the `Brush` suffix.** A `Color` cannot be seated in an `IBrush` property — the renderer throws `InvalidCastException` (verified against the live pipeline). It fails loudly, so you will never ship it unnoticed, but there is no scenario where the bare form helps here. | ✅ **Write the bare form.** This is its home: `Color="{color:AccentNeutral}"`. |
| **Overlay tints** (`OverlayWeakBrush`, …) | *no bare form exists* | Keep the suffix — it is the only form. | Same. |

House rule: **bare by default wherever a bare form exists and the slot accepts it; the `Brush` suffix survives only where it is load-bearing** — plain tokens feeding paint properties, and overlays everywhere. The positional `AccentBrush` extension (`{color:AccentBrush Dark1}`) is already minimal; never append a stage to a role or plain token. `ShorthandSafetyTests` pins all three behaviours, so widening this rule without updating those tests will fail the build.

> **Never use an `Overlay*` brush as a `Background` for a state.** `Background` is a single property, so a translucent value *replaces* the fill rather than layering over it, and whatever is behind the control shows through. It reads correctly only while the control and the page are the same color. Overlays are for shadows and scrims, drawn as their own `Border` behind the target — see below.
...
| Token | Opacity | Use |
|-------|---------|-----|
| `{color:OverlayWeakBrush}` | 8% | Subtle depth, card shadows, hover lifts |
| `{color:OverlayMediumBrush}` | 16% | Standard elevation, dropdown panels |
| `{color:OverlayStrongBrush}` | 32% | High elevation, modals, focused elements |
...
- **`BoxShadow` strings** (e.g. `BoxShadow="0 6 8 0 #4F000000"`): Avalonia's `BoxShadow` is a string property and cannot be data-bound to theme tokens. **Replace the `BoxShadow` entirely** with a `Border` element positioned behind the target element:
  ...
  <Border Background="{color:OverlayWeakBrush}"
          CornerRadius="{size:RadiusMd}"
          Margin="0, 0, -4, -4"
          ClipToBounds="False"/>
  <!-- Main card content -->
  <Border Background="{color:AccentBrush Light10}"
          CornerRadius="{size:RadiusMd}">
      ...
  </Border>
  ```
...
- **Gradient shadows** (`LinearGradientBrush`, `RadialGradientBrush`): Use theme tokens in every `GradientStop`. For a fade shadow:
  ...
  <LinearGradientBrush StartPoint="0,0" EndPoint="0,1">
      <GradientStop Offset="0" Color="{color:AccentNeutral}" Opacity="0.12"/>
      <GradientStop Offset="1" Color="{color:AccentNeutral}" Opacity="0"/>
  </LinearGradientBrush>
  ```
  Or use `{color:OverlayWeakBrush}` for shadows that should invert between light/dark themes.
...
When you find any hard-coded color value, apply this test **before** deciding whether it is exempt:

> **❓ "If the user swaps to a dark theme or a colorful accent theme, would this value need to change?"**
>
...
**Statement:** Every `Setter` at the `ControlTheme` level (i.e. direct children of the `<ControlTheme>` element) that sets a visual property MUST source its value from a theme token (`{theme:...}`) or a `{StaticResource}` defined within the same file. No hard-coded colors, sizes, brushes, or thicknesses at the ControlTheme level.
...
**Reference example:** The Button control uses `<Style Selector="^:pointerover">` with `{color:OverlayWeakBrush}` for hover and `<Style Selector="^:pressed">` with `{color:OverlayStrongBrush}` for pressed — these are setters on the Button's ControlTheme, not inside the template. Similarly, the CalendarView uses `^ /template/` styles on its zoom-grid and picker parts to switch their visibility per pseudo-class state.

---

## Rule 4: Every Visual Property on the Control Should Be Exposed as a Setter in the ControlTheme

**Statement:** If a visual aspect of a control can be styled (color, thickness, corner radius, font, padding, alignment, etc.), it SHOULD have a corresponding `Setter` at the ControlTheme level that maps to a theme token. Do not bury visual constants inside the template or in inner styles.
...
<!-- Opening a Popup detaches this calendar into a separate host subtree, which severs
     inherited properties — so the accent hue must be forwarded explicitly or every
...
**Reference violation (the case that produced this rule):** `PathIcon` set `Foreground` to `{color:AccentBrush Dark1}` in its visual defaults. Every icon in the library was therefore nailed to the neutral color. When `ButtonSpinner` set `Foreground` to `{theme:AccentPrimaryDark1Brush}` on its spinner buttons for hover, the chevron glyph did not change — the icon's own setter (priority 3) outranked the inherited value (priority 4). The fix was to delete the setter, not to add a louder override in `ButtonSpinner`. One deletion made every `PathIcon` in the library accent-aware, state-aware and dark-variant-aware at once, because they all now resolve through their container.

Note the shape of the trap: the wrong fix (push the color down from each container that needs it) appears to work for the control in front of you and has to be repeated forever. The right fix (stop blocking inheritance) is a deletion.
...
- Restating the ambient value in a control's visual defaults — `Foreground="{color:AccentBrush Dark1}"`, `FontFamily="{theme:FontFamily}"`, `FontSize="{size:FontSizeMd}"`, `FontWeight="{theme:FontWeightRegular}"` on a control that sits inside a themed root. This is the common case and it is always wrong.
- Any decoration or content-rendering element that has no color identity of its own — icons, glyphs, presenters, adorners. These exist to be colored by their container.
...
**How to detect:** For every `<Setter>` on an inherited property that sits in a ControlTheme's *visual defaults* (not inside a `<Style>`), ask: *if I delete this, what color/font does the control get?* If the answer is "the same one", it must be deleted. If you cannot answer without running the app, the property is being set in more places than it should be.
...
**Statement:** Bind colors with `{color:AccentBrush Stage}`, never with a specific hue like `{color:AccentPrimaryBrush}`. One block then serves every hue instead of being copy-pasted per color. Pin a hue only where the meaning demands it — a focus ring is always `{color:AccentFocusBrush}`.

**Why:** `{color:AccentBrush}` resolves `Accent{Hue}{Shade}Brush` at runtime, against whichever hue the *target control* is carrying. Naming `AccentPrimaryBrush` pins the block to one accent, so supporting warning, success and the rest means duplicating thirty lines apiece — the exact problem the extension exists to remove.

**Correct:**
```xml
<Style Selector="^:pointerover">
  <Setter Property="Foreground" Value="{color:AccentBrush}" />
  <Setter Property="BorderBrush" Value="{color:AccentBrush}" />
</Style>
...
<Style Selector="^:pointerover">
  <Setter Property="Foreground" Value="{color:AccentPrimaryBrush}" />
</Style>
```
...
`AvaloniaThemeLab/BrandControls.axaml` is the worked example of the consumer side: it layers `.hero` and `.quiet` over the theme's Button using plain top-level selectors, needs no declarations file of its own, and still reads `{color:AccentBrush}` so one class serves every hue. It is deliberately *not* a `ControlTheme` — that would mean `BasedOn="{StaticResource {x:Type Button}}"`, and the control dictionaries here are `x:ClassModifier="internal"`, so a consumer cannot reach them. Layering over the ControlTheme also means a brand button keeps the theme's template and states for free.
...
- `AccentFocus` / `AccentFocusBrush` — a dedicated focus-indicator color (distinct from accent primary)
