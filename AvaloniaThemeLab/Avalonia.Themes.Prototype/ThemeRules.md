...
This Prototype theme replaces that with a **token-based architecture**. A single `DefaultTheme.cs` class defines every design decision — colours, spacing, type scale, corner radii, control heights, thicknesses — as named tokens. These tokens are exposed as markup extensions (`{theme:AccentBrush Light10}`, `{theme:SpacingMd}`, `{theme:RadiusSm}`, etc.) that any control theme can reference.

The effect is transformative:

- **Clean, concise XAML syntax.** Instead of referencing resources through string-based keys (`{StaticResource MyColour}`) with no compile-time checking, you write `{theme:AccentBrush Light10}`, `{theme:SpacingMd}`, `{theme:RadiusSm}`. This is valid XAML that the AngelSix.ThemeEngine source generator understands directly.
...
|-----------|---------|-----|
| **`LinearGradientBrush` / `RadialGradientBrush` with hardcoded stops** | `<GradientStop Offset="0" Color="#4F000000"/>` | Every `GradientStop.Color` must use a theme token: `Color="{theme:AccentNeutral}"` (with opacity on the brush or stop) |
| **`SolidColorBrush` with hardcoded hex** | `<SolidColorBrush Color="#FF797774"/>` | Use `Color="{theme:...}"` — e.g. `Color="{theme:AccentNeutral}"` |
| **`BoxShadow` with any hardcoded colour** | `BoxShadow="0 6 8 0 #4F000000"` | Replace with a `Border` element behind the target using `Background="{theme:OverlayWeakBrush}"` (or another overlay brush). See [Shadow Theming](#shadow-theming) below. |
| **Any `#RRGGBB` or `#AARRGGBB` hex on a UI element** | `Fill="#FF000000"`, `Stroke="#80FFFFFF"` | Replace with the appropriate theme brush token |

> **Avalonia caveat:** `GradientStop` has no `Opacity` property — attempting `<GradientStop Opacity="0.05"/>` produces `AVLN2000`. Use a themed brush with built-in opacity (e.g., `{theme:OverlayWeakBrush}`) instead, or encode the alpha in the colour itself via a token.
...
```xml
<Setter Property="Background"  Value="{theme:AccentBrush Light10}" />
<Setter Property="Foreground"  Value="{theme:AccentBrush Dark1}" />
<Setter Property="BorderBrush" Value="{theme:AccentBrush Dark1}" />
<Style Selector="^:pointerover">
  <Setter Property="Background" Value="{theme:AccentBrush Light8}" />
</Style>
```
...
**Declaring a hue is all-or-nothing.** The hues carry `[AccentHue]`, which puts the generator in explicit mode: any hue *anywhere*, including in a downstream theme, must then carry it too or it silently drops out of the `AccentKind` enum. `AccentBorder` and `AccentFocus` deliberately lack it — they are role colours, reached as `{theme:AccentBorderBrush}`, not hues a control can *be*.
...
**Neutral is reachable directly, and that is a different statement.** `{theme:AccentBrush Dark1}` follows whatever `Accent.Kind` the control carries; `{theme:NeutralDark1Brush}` is always the neutral ramp. The `Neutral*` family exists for the parts of an accented control that must *not* take the hue — a caret, a disabled label, a divider, a scrollbar groove.

| You mean | Use |
|---|---|
| "this follows the control's hue" | `{theme:AccentBrush Dark1}` |
...
| Token | Opacity | Use |
|-------|---------|-----|
| `{theme:OverlayWeakBrush}` | 8% | Subtle depth, card shadows, hover lifts |
| `{theme:OverlayMediumBrush}` | 16% | Standard elevation, dropdown panels |
| `{theme:OverlayStrongBrush}` | 32% | High elevation, modals, focused elements |
...
- **`BoxShadow` strings** (e.g. `BoxShadow="0 6 8 0 #4F000000"`): Avalonia's `BoxShadow` is a string property and cannot be data-bound to theme tokens. **Replace the `BoxShadow` entirely** with a `Border` element positioned behind the target element:
  ...
  <Border Background="{theme:OverlayWeakBrush}"
          CornerRadius="{theme:RadiusMd}"
          Margin="0, 0, -4, -4"
          ClipToBounds="False"/>
  <!-- Main card content -->
  <Border Background="{theme:AccentBrush Light10}"
          CornerRadius="{theme:RadiusMd}">
      ...
  </Border>
  ```
...
- **Gradient shadows** (`LinearGradientBrush`, `RadialGradientBrush`): Use theme tokens in every `GradientStop`. For a fade shadow:
  ...
  <LinearGradientBrush StartPoint="0,0" EndPoint="0,1">
      <GradientStop Offset="0" Color="{theme:AccentNeutral}" Opacity="0.12"/>
      <GradientStop Offset="1" Color="{theme:AccentNeutral}" Opacity="0"/>
  </LinearGradientBrush>
  ```
  Or use `{theme:OverlayWeakBrush}` for shadows that should invert between light/dark themes.
...
**Reference example:** The Button control uses `<Style Selector="^:pointerover">` with `{theme:OverlayWeakBrush}` for hover and `<Style Selector="^:pressed">` with `{theme:OverlayStrongBrush}` for pressed — these are setters on the Button's ControlTheme, not inside the template. Similarly, the CalendarItem uses `<Style Selector="^ /template/ Button:pointerover > Path">` to update the nav arrow stroke colour on hover.
...
- Use `{theme:OverlayWeakBrush}` / `{theme:OverlayMediumBrush}` / `{theme:OverlayStrongBrush}` for hover/pressed/disabled states instead of adding per-control accent tokens.
...
| The control must contrast with its own background | A selected item on an accent fill → `{theme:AccentBrush Light10}` |
| A **state or class** style changes it | `^:disabled` → `{theme:AccentBrush Light2}`, `^:pointerover` → `{theme:AccentPrimaryDark1Brush}` |
...
- Restating the ambient value in a control's visual defaults — `Foreground="{theme:AccentBrush Dark1}"`, `FontFamily="{theme:FontFamily}"`, `FontSize="{theme:FontSizeMd}"`, `FontWeight="{theme:FontWeightRegular}"` on a control that sits inside a themed root. This is the common case and it is always wrong.
...
**Statement:** Bind colours with `{theme:AccentBrush Stage}`, never with a specific hue like `{theme:AccentPrimaryBrush}`. One block then serves every hue instead of being copy-pasted per colour. Pin a hue only where the meaning demands it — a focus ring is always `{theme:AccentFocusBrush}`.
...
`AvaloniaThemeLab/BrandControls.axaml` is the worked example of the consumer side: it layers `.hero` and `.quiet` over the theme's Button using plain top-level selectors, needs no declarations file of its own, and still reads `{theme:AccentBrush}` so one class serves every hue. It is deliberately *not* a `ControlTheme` — that would mean `BasedOn="{StaticResource {x:Type Button}}"`, and the control dictionaries here are `x:ClassModifier="internal"`, so a consumer cannot reach them. Layering over the ControlTheme also means a brand button keeps the theme's template and states for free.
...
| **Colour ramps** | One hue per `[AccentHue] Color Accent{Hue}`, each with `Light1..10` / `Dark1..10` from `ColorRamp`. Reached as `{theme:AccentBrush Dark2}`; the stage properties have no extension of their own |
